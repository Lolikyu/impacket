...
#
# Description:
#   This script will gather the description of the AD accounts (only if description and sAMAccountName are not empty)
#
# Author:
#   Alberto Solino (@agsolino)
#   Charlie Bromberg (@_nwodtuhs)
#   Nicolas Ranc (@pathtaga)
#
# Reference for:
#   LDAP
#
...
class GetADUsers:
    ...
    def processRecord(self, item):
        ...
        try:
            ...
            if sAMAccountName != "" and description != "":
                print("%s,%s" % (sAMAccountName, description))
    ...
    def run(self):
        ...
        logging.debug('Querying %s for information about domain.' % self.__target)

        # Building the search filter
        if self.__all:
            searchFilter = "(&(sAMAccountName=*)(objectCategory=*)"
        else:
            searchFilter = "(&(sAMAccountName=*)(description=*)(!(UserAccountControl:1.2.840.113556.1.4.803:=%d))" % UF_ACCOUNTDISABLE
...
if __name__ == '__main__':
    ...
    parser.add_argument('-all', action='store_true', help='Query the description of all accounts, including the disabled ones')
